...
// FHE utility functions (simplified version for script)
class FHEUtils {
  ...
  static generateCategoryHash(name, description) {
    const crypto = require('crypto');
  ...
  static generateAuthorHash(name, email) {
    const crypto = require('crypto');
  ...
  static generateContentHash(content, title) {
    const crypto = require('crypto');
    const combined = content + title + Date.now().toString();
    const hash = crypto.createHash('sha256').update(combined).digest('hex');
    return '0x' + hash;
  }
}
